...
def evaluate_bdt(df: ak.Record, variation, model, training_features: List[str], parameters) -> ak.Record :
    ...
    # features = prepare_features(df,training_features, variation=variation, add_year=False) # converts copperheadV1 features to copperheadV2 features I believe
    features = training_features
    print(f"evaluate_bdt features: {features}")
    print(f"evaluate_bdt df: {df.fields}")
